"""
Unhide column P in the TURF SUPPLY spreadsheet (correct one).
Target: 1aygUdgHPMuI14uiGfqZdDFIvBHMmarPwbdfdONml_UI

Usage:
    python -m scripts.unhide_p_turf_supply
"""
import sys
import json
import re
...
def force_show_column_p(service, spreadsheet_id, sheet_id):
    ...
    # Make P visible with 200px width
    requests_step2.append({
        'updateDimensionProperties': {
            'range': {
                'sheetId': sheet_id,
...
def main():
    print("=" * 80)
    print("UNHIDING COLUMN P IN TURF SUPPLY SPREADSHEET")
    print("=" * 80)
    print()

    service = get_service()

    # USE THE CORRECT SPREADSHEET ID (Turf Supply, not Sales)
    spreadsheet_id = settings.google_spreadsheet_id  # Changed from sales_spreadsheet_id

    print(f"Target Spreadsheet ID: {spreadsheet_id}")
    print(f"Expected: 1aygUdgHPMuI14uiGfqZdDFIvBHMmarPwbdfdONml_UI")

    if spreadsheet_id == "1aygUdgHPMuI14uiGfqZdDFIvBHMmarPwbdfdONml_UI":
        print("OK: Correct spreadsheet")
    else:
        print("WARNING: Wrong spreadsheet!")
    print()
    ...
    print()
    print("Column P should now be visible (200px width).")
